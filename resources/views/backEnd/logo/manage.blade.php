                ...
                <tr>
                  ...
                  <td>
                  	<ul class="action_buttons">
                  		<li>
                  			@if($value->status==1)
                  			<form action="{{url('editor/logo/unpublished')}}" method="POST">
                  				@csrf
                  				<input type="hidden" name="hidden_id" value="{{$value->id}}">
                  				<button type="submit" class="thumbs_up" title="unpublished"><i class="fa fa-thumbs-up"></i></button>
                  			</form>
                  			@else
	                  			<form action="{{url('editor/logo/published')}}" method="POST">
	                  				@csrf
	                  				<input type="hidden" name="hidden_id" value="{{$value->id}}">
                  		...
                  		<li>
                  			<a class="edit_icon" href="{{url('editor/logo/edit/'.$value->id)}}" title="Edit"><i class="fa fa-edit"></i></a>
                  		</li>
                  		<li>
